...
class TestSetupPy:
    ...
    def test_post_install_command_git_repo(self, mock_exists, mock_run):
        ...
        try:
            ...
            git_calls = [
                call for call in mock_run.call_args_list if "git submodule" in str(call)
            ]
    ...
    def test_post_install_command_no_git(self, mock_exists, mock_run):
        ...
        try:
            ...
            mock_run.assert_called()
            clone_calls = [
                call for call in mock_run.call_args_list if "git clone" in str(call)
            ]
            assert len(clone_calls) > 0

            # Verify it's using the forked repositories
            clone_commands = [str(call) for call in clone_calls]
            assert any("discolotus/slsk-batchdl" in cmd for cmd in clone_commands)
            assert any("discolotus/Shazam-Tool" in cmd for cmd in clone_commands)
...
class TestBuildProcess:
    ...
    @patch("os.makedirs")
    @patch("os.chmod")
    def test_slsk_build_process_macos_arm64(
        self, mock_chmod, mock_makedirs, mock_exists, mock_run
    ):
...
class TestProjectStructure:
    ...
    def test_external_tools_structure(self):
        ...
        expected_dirs = ["Shazam-Tool", "slsk-batchdl", "bin"]
...
class TestInstallationProcess:
    ...
    def test_pip_install_simulation(self, mock_run):
        ...
        result = subprocess.run(
            ["echo", "pip install -e ."], capture_output=True, text=True
        )
