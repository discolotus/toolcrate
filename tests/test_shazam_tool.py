...
class TestShazamTool:
    ...
    async def test_get_name_with_retries(self):
        ...
        with (
            patch("shazam.Shazam") as mock_shazam_class,
    ...
    def test_process_downloads(self, temp_dir):
        ...
        with (
            patch("shazam.DOWNLOADS_DIR", str(downloads_dir)),
    ...
    def test_main_scan_command(self, mock_process):
        ...
        with patch("sys.argv", test_args), patch("shazam.setup_logging"):
    ...
    def test_process_audio_file(
        self, mock_get_name, mock_remove, mock_segment, temp_dir
    ):
        ...
        mock_get_name.side_effect = asyncio.coroutine(
            lambda x: "Artist - Song" if "1.mp3" in x else "Not found"
        )
