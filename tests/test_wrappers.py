...
class TestGetProjectRoot:
    ...
    def test_get_project_root_found(self, mock_project_root):
        ...
        with (
            ...
            mock_abspath.return_value = str(
                mock_project_root / "src" / "toolcrate" / "cli" / "wrappers.py"
            )
...
class TestRunSlsk:
    ...
    def test_run_slsk_local_binary_found(
        self, mock_file_system, mock_os_execv, mock_logger
    ):
        ...
        with (
            ...
            patch(
                "toolcrate.cli.wrappers.get_project_root",
                return_value=mock_file_system["root"],
            ),
            ...
            mock_os_execv.assert_called_once_with(
                str(binary_path), ["sldl", "search", "test"]
            )
    ...
    def test_run_slsk_docker_image_found(
        self, mock_subprocess, mock_os_execvp, mock_logger
    ):
        ...
        with (
            ...
            expected_cmd = [
                ...
                "-it",
                "-v",
                f"{os.path.expanduser('~/Music/downloads')}:/downloads",
                "slsk-batchdl",
                "search",
                "test",
            ]
...
class TestRunShazam:
    ...
    def test_run_shazam_python_script_found(
        self, mock_file_system, mock_subprocess, mock_logger
    ):
    ...
    def test_run_shazam_shell_script_found(
        self, mock_file_system, mock_subprocess, mock_logger
    ):
    ...
    def test_run_shazam_path_binary_found(
        self, mock_shutil, mock_os_execvp, mock_logger
    ):
        ...
        with (
            ...
            mock_os_execvp.assert_called_once_with(
                "shazam-tool", ["shazam-tool", "scan"]
            )

    def test_run_shazam_docker_image_found(
        self, mock_subprocess, mock_os_execvp, mock_logger
    ):
        ...
        with (
            ...
            expected_cmd = [
                ...
                "-it",
                "-v",
                f"{os.path.expanduser('~/Music')}:/music",
                "shazam-tool",
                "scan",
            ]
            mock_os_execvp.assert_called_once_with("docker", expected_cmd)
...
class TestRunMdl:
    ...
    def test_run_mdl_native_binary_found(
        self, mock_shutil, mock_os_execvp, mock_logger
    ):
        ...
        with patch("sys.argv", ["mdl-tool", "get-metadata", "file.mp3"]):
            ...
            mock_os_execvp.assert_called_once_with(
                "mdl-utils", ["mdl-utils", "get-metadata", "file.mp3"]
            )
    ...
    def test_run_mdl_docker_image_found(
        self, mock_subprocess, mock_os_execvp, mock_logger
    ):
