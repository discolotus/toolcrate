...
class TestVerifyExecutable:
    ...
    def test_returns_false_on_timeout(self, tmp_path):
        ...
        with patch(
            "subprocess.run",
            side_effect=subprocess.TimeoutExpired(cmd="sldl", timeout=15),
        ):
            assert _verify_executable(binary) is False
...
class TestEnsureSldlBinary:
    def test_returns_existing_valid_binary(self, tmp_path):
        ...
        with (
            patch("toolcrate.cli.binary_manager.get_binary_path", return_value=binary),
    ...
    def test_force_refresh_re_downloads(self, tmp_path):
        ...
        with (
            ...
            patch("toolcrate.cli.binary_manager._verify_executable", return_value=True),
            patch(
                "toolcrate.cli.binary_manager.resolve_latest_version",
                return_value="v1.0",
            ),
            patch(
                "toolcrate.cli.binary_manager._install_from_release",
                return_value=binary,
            ) as mock_install,
        ):
    ...
    def test_build_from_source_env_var(self, tmp_path):
        ...
        with (
            patch.dict(os.environ, {"TOOLCRATE_SLDL_BUILD_FROM_SOURCE": "1"}),
            patch(
                "toolcrate.cli.binary_manager.get_binary_path",
                return_value=tmp_path / "sldl",
            ),
            patch(
                "toolcrate.cli.binary_manager._verify_executable", return_value=False
            ),
            patch(
                "toolcrate.cli.binary_manager._build_from_source", return_value=binary
            ) as mock_build,
        ):
    ...
    def test_falls_back_to_source_when_download_fails(self, tmp_path):
        ...
        with (
            ...
            patch(
                "toolcrate.cli.binary_manager.resolve_latest_version",
                side_effect=BinaryError("no network"),
            ),
            patch(
                "toolcrate.cli.binary_manager._build_from_source", return_value=binary
            ) as mock_build,
